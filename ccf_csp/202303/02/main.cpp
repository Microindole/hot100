...
1.  **你的逻辑：** 看到第一根 7 最高，锯掉 1 米（花 1 元），高度变成 `[6, 7,
5]`。此时总高度还是
**7**（第二根拖了后腿）。
2.  **二分的逻辑：** 它不关心你是怎么锯的。它直接问：“我想把总高度降到 **5**
米，行不行？”
    * 第一根：从 7 降到 5，需要锯 2 米，花 $2 \times 1 = 2$ 元。
    * 第二根：从 7 降到 5，需要锯 2 米，花 $2 \times 2 = 4$ 元。
    * 第三根：已经是 5 了，不用锯。
    * **结论：** 总共要花 $2 + 4 = 6$ 元。如果你的钱 $m \ge 6$，那“降到 5
米”这个目标就**可行**。
...
2.  **验证 `mid` 行不行**：
    * 遍历一遍所有区域。
    * 只要某个区域耗时 $t_i > mid$，就计算把这块区域缩减到 $mid$
天要花多少钱：$(t_i - mid) \times c_i$。
    * 把所有这些钱加起来，看是否 $\le$ 你手里的 $m$。
3.  **调整范围**：
    * 如果**行**：说明 `mid`
天可以实现。但这不一定是最小的，我们尝试更小的天数，把范围缩小到 $[k, mid-1]$。
    * 如果**不行**：说明钱不够，`mid` 天做不到。我们必须增加天数，把范围缩小到
$[mid+1, \text{max}(t_i)]$。
*/
